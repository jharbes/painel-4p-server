...
public interface DetailProductRepository extends JpaRepository<DetailProduct, Long>{
	...
	@Query(value = "select d.id from detailproduct d ,datafile d2, datafile_detailproduct dd "
			+ "where d2.id = dd.datafile_id  and dd.datafile_detailproduct_id = d.id and d2.id= :idDatafile ",nativeQuery = true)
	...
	@Query(value = "select distinct  d.\"data\", pd.\"name\" as p ,s.\"name\" as s,dp.validity,dp.stock \r\n"
			+ "FROM datafile d, operation.shop s, operation.promoter p, detailproduct dp , datafile_detailproduct dd, product pd "
			+ "where d.brand_id = :idBrand and d.shop_id = s.id \r\n"
			+ "and d.id = dd.datafile_id  and dd.datafile_detailproduct_id = dp.id and dp.product_id = pd.id \r\n"
			+ "and dp.validity >= :finalDate and dp.validity <= :validityFinalDate  and dp.stock > 0 and \r\n"
			+ "d.\"data\" = (SELECT max(d2.data) \r\n"
			+ "FROM datafile d2, operation.shop s2, operation.promoter p2, detailproduct dp2 , datafile_detailproduct dd2, product pd2\r\n"
			+ "where d2.brand_id = :idBrand and d2.shop_id = s2.id \r\n"
			+ "and d2.id = dd2.datafile_id  and dd2.datafile_detailproduct_id = dp2.id and dp2.product_id = pd2.id \r\n"
